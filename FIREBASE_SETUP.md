...
export const GROUP_ID = "mehmed-crew";   // everyone in your group uses this same value
```
...
- **Ride requests** — when you tap **Send to the crew** on the Ride tab, every crewmate with the app open gets a **notification** ("needs a ride · Home → Work · Tonight"). They must tap **Enable** on their Me screen once to allow alerts.
...
Ride-request alerts pop on any crew phone that has the app **open in the background** (a live tab or the installed PWA running). Delivering a push to a phone whose app is fully **closed** needs Firebase Cloud Messaging (a server key + a bit more setup) — a good next step, but the in-app alerts above work with no extra config.
